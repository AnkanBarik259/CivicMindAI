const fs = require('fs');
const nav = fs.readFileSync('src/pages/AIRouteNavigator.tsx', 'utf-8');
const map = fs.readFileSync('src/components/global/MapComponent.tsx', 'utf-8');

// The new MapComponent needs to be the map page but with a collapsible panel over it.
// We'll construct it string by string.
...
export default function MapComponent() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.routes && data.routes[0]) {
          ...
          const currentNearbyHazards = validReports.filter(r => isPointNearRoute({ lat: r.location.latitude, lng: r.location.longitude }, coords));
          ...
          if (data.routes.length > 1) {
             for (let i = 1; i < data.routes.length; i++) {
                 ...
                 const altHazards = validReports.filter(r => isPointNearRoute({ lat: r.location.latitude, lng: r.location.longitude }, altCoords));
          ...
          setIsAnalyzing(true);
          try {
             const analysis = await api.ai.analyzeRoute(primaryRouteData, alternativeRoutesData);
  ...
  const startNavigation = () => {
    ...
    const id = navigator.geolocation.watchPosition((pos) => {
      ...
      if (routePath && routePath.length > 0) {
         if (!isPointNearRoute(currentLoc, routePath, 0.001)) {
  ...
  return (
    <div className="w-full h-full relative flex flex-col z-0 overflow-hidden">
          ...
          <AnimatePresence>
            {isPanelOpen && (
              ...
              >
                ...
                {routePath && !isLoadingRoute && (
                  <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg border border-slate-100 dark:border-slate-800">
                        <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">{isNavigating ? 'Rem Dist' : 'Total Dist'}</p>
                      ...
                      <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg border border-slate-100 dark:border-slate-800">
                        ...
                        <p className="text-lg font-bold text-slate-900 dark:text-white">{isNavigating ? formatDuration(remainingDistance / routeDistance * routeDuration) : formatDuration(routeDuration)}</p>
                      </div>
                      
                      {routeAnalysis?.estimatedDelaySeconds && (
                        <>
                          <div className="bg-amber-50 dark:bg-amber-900/20 p-3 rounded-lg border border-amber-100 dark:border-amber-800">
                            <p className="text-[10px] text-amber-600 dark:text-amber-400 uppercase font-bold tracking-wider mb-1">Est. Delay</p>
                            <p className="text-lg font-bold text-amber-700 dark:text-amber-500">+{formatDuration(isNavigating ? remainingETA - (remainingDistance / routeDistance * routeDuration) : routeAnalysis.estimatedDelaySeconds)}</p>
                          </div>
                          <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-100 dark:border-blue-800">
                            <p className="text-[10px] text-blue-600 dark:text-blue-400 uppercase font-bold tracking-wider mb-1">{isNavigating ? 'Rem ETA (Adj)' : 'AI Adj ETA'}</p>
                    ...
                    {!isNavigating && (
                      <button
                        ...
                        className="w-full mt-2 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-lg shadow-sm transition-colors flex items-center justify-center gap-2 text-sm"
                      >
                        <Navigation2 className="w-4 h-4" />
                ...
                {/* AI Analysis Block inside panel */}
                {routePath && (
                    ...
                    {isAnalyzing ? (
                      ...
                      <div className="space-y-3">
                        <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg border border-slate-100 dark:border-slate-800">
                          ...
                          <p className={\`text-xl font-bold flex items-center gap-2 \${routeAnalysis.roadHealthScore >= 80 ? 'text-emerald-500' : routeAnalysis.roadHealthScore >= 50 ? 'text-yellow-500' : 'text-red-500'}\`}>
                            {routeAnalysis.roadHealthScore}/100 
                            <span className="text-xs font-medium">
                              {routeAnalysis.roadHealthScore >= 80 ? '🟢 Excellent' : routeAnalysis.roadHealthScore >= 50 ? '🟡 Moderate' : '🔴 Poor'}
                            </span>
                          </p>
                        </div>
                        ...
                        {routeAnalysis.hasSaferAlternative && routeAnalysis.saferAlternative && (
                          <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg p-3">
                            <h4 className="font-bold text-emerald-800 dark:text-emerald-400 mb-1 flex items-center gap-1 text-xs">
                              <Navigation2 className="w-3 h-3" /> Safer Route
                            </h4>
                            <p className="text-xs text-emerald-700 dark:text-emerald-300 mb-2">
                              {routeAnalysis.saferAlternative.reason}
                            </p>
                            <div className="flex gap-3 text-xs font-bold text-emerald-800 dark:text-emerald-300">
                              <span>+{routeAnalysis.saferAlternative.extraDistance}</span>
                              <span>+{routeAnalysis.saferAlternative.extraTime}</span>
                            </div>
                          </div>
                        )}
                      </div>
      ...
      {/* Map Legend */}
      <div className="absolute bottom-6 left-6 z-[1000] bg-white/90 dark:bg-slate-900/90 backdrop-blur-md p-3 rounded-xl shadow-lg border border-slate-200 dark:border-slate-800 text-xs pointer-events-none">
        <h4 className="font-bold text-slate-900 dark:text-white mb-2 uppercase tracking-wider">Map Legend</h4>
...
fs.writeFileSync('src/components/global/MapComponent.tsx', newFile);
